...
export async function createLugar(lugar) {
  const { data, error } = await supabase
    .from('lugares')
    .insert(lugar)
    .select()
...
export async function getAllVias() {
  const { data, error } = await supabase
    .from('vias')
    .select('*, lugares(nombre), grupos(nombre)')
...
export async function getVia(id) {
  const { data, error } = await supabase
    .from('vias')
    .select('*, grupos(id, nombre, lugar_id, auto_url, acampe_url, acampe_obs, altitud, imagen, lugares(id, nombre, zona, maps_url, imagen))')
    .eq('id', id)
    .single()
  if (error) throw error
  return data
...
export async function createVia(via) {
  const { data, error } = await supabase
    .from('vias')
    .insert(via)
    .select()
